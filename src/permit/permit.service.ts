...
import { PermitCreate } from './guards/permit.guard';
...
export class PermitService {
     ...
     * 
     */
    public async create ({ data }: { data:PermitCreate }) {
        try {
            // Inicio
            const dataCreate: Prisma.userGroupCreateInput = {
                name: data.name,
                group: data.group
            }

            if(data.userId) {
                dataCreate.createByRef = { connect:{ id:data.userId } }
            }
     ...
     * @returns 
     */
    public async paginate ({ filter, skip, take }: { filter:Prisma.userGroupWhereInput, skip:number, take: number }) {
        try {
            // Inicio
     ...
     * @returns 
     */
    public async find ({ filter }: { filter:Prisma.userGroupWhereInput }) {
        try {
            // Inicio
     ...
     * @returns 
     */
    public async udpate ({ id,data }: { id:string,data:PermitCreate }) {
        try {
            // Inicio
